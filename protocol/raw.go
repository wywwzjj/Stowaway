...
func (message *RawMessage) ConstructData(header *Header, mess interface{}, isPass bool) {
	...
	if !isPass {
		...
		case FILEDOWNRES:
			...
			dataBuffer.Write(OKBuf)
		case FILELSREQ:
			mmess := mess.(*DirStatReq)
			dirNameLenBuf := make([]byte, 4)
			binary.BigEndian.PutUint32(dirNameLenBuf, mmess.DirNameLen)

			dataBuffer.Write(dirNameLenBuf)
			dataBuffer.WriteString(mmess.DirName)

		case FILELSRES:
			mmess := mess.(*DirStatRes)

			dirNameLenBuf := make([]byte, 4)
			binary.BigEndian.PutUint32(dirNameLenBuf, mmess.FilesLen)
			dataBuffer.Write(dirNameLenBuf)

			for _, file := range mmess.Files {
				filenameLenBuf := make([]byte, 4)
				binary.BigEndian.PutUint32(filenameLenBuf, file.FilenameLen)

				filenameBuf := []byte(file.Filename)

				fileSizeBuf := make([]byte, 8)
				binary.BigEndian.PutUint64(fileSizeBuf, file.FileSize)

				sliceNumBuf := make([]byte, 8)
				binary.BigEndian.PutUint64(sliceNumBuf, file.SliceNum)
...
func (message *RawMessage) DeconstructData() (*Header, interface{}, error) {
	...
	case FILEDOWNRES:
		...
		return header, mmess, nil
	case FILELSREQ:
		mmess := new(DirStatReq)
		mmess.DirNameLen = binary.BigEndian.Uint32(dataBuf[:4])
		mmess.DirName = string(dataBuf[4 : 4+mmess.DirNameLen])
		return header, mmess, nil
	case FILELSRES:
		mmess := new(DirStatRes)
		mmess.FilesLen = binary.BigEndian.Uint32(dataBuf[:4])

		for i := 0; i < int(mmess.FilesLen); i++ {
			f := new(FileStatReq)
			f.FileSize = 10
			f.FilenameLen = 10
			f.Filename = "testtest"
...
func (message *RawMessage) SendMessage() {
	...
	n, err := message.Conn.Write(finalBuffer)
	if err != nil {
		fmt.Printf("send %d\n", n)
		panic(err)
	}
