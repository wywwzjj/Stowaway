...
func (file *MyFile) ListDir(route string, targetUUID string, filePath string) {
	sMessage := protocol.PrepareAndDecideWhichSProtoToLower(global.G_Component.Conn, global.G_Component.Secret, global.G_Component.UUID)

	dir := &protocol.DirStatReq{
		DirNameLen: uint32(len(filePath)),
		DirName:    filePath,
	}
	...
	header := &protocol.Header{
		...
		Accepter:    targetUUID,
		MessageType: protocol.FILELSREQ,
		RouteLen:    uint32(len([]byte(route))),
		Route:       route,
	}

	protocol.ConstructMessage(sMessage, header, dir, false)
	sMessage.SendMessage()
}


func (file *MyFile) SendDirStat(route string, targetUUID string) error {
	sMessage := protocol.PrepareAndDecideWhichSProtoToUpper(global.G_Component.Conn, global.G_Component.Secret, global.G_Component.UUID)

	statHeader := &protocol.Header{
		Sender:      global.G_Component.UUID,
		Accepter:    targetUUID,
		MessageType: protocol.FILELSRES,
		RouteLen:    uint32(len([]byte(route))),
		Route:       route,
	}

	fileInfos, err := ioutil.ReadDir(file.FilePath)
	if err != nil {
		return err
	}

	dirStatRes := &protocol.DirStatRes{
		FilesLen: uint32(len(fileInfos)),
		Files:    nil,
	}
	...
	for _, fileInfo := range fileInfos {
		...
		dirStatRes.Files = append(dirStatRes.Files, fileStatReqMess)
	}

	protocol.ConstructMessage(sMessage, statHeader, dirStatRes, false)
	sMessage.SendMessage()
